...
setTimeout(function patchPDF(){
  ...
  window.exportSessionPDF=function(){
    ...
    const html='<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>Seance '+fmtD(s.date)+'</title>'
      ...
      +'<script>function _dl(){const b=new Blob([document.documentElement.outerHTML],{type:\'text\/html;charset=utf-8\'});const u=URL.createObjectURL(b);const a=document.createElement(\'a\');a.href=u;a.download=\'seance_'+s.date.+'\'.html\';a.click();setTimeout(()=>URL.revokeObjectURL(u),2000);}<\/script>'
      +'</body></html>';
